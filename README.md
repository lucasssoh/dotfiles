...
| **Balise** (`config/hyprland/balise-src/` + `quickshell/bar/modules/balise/`) | WiFi/Bluetooth/Ethernet panel — first-party, replacing the vendored Orbit it started from. Now split in two: a Rust daemon (NetworkManager/BlueZ behind a Unix socket) and a QML panel in the bar that talks to it. The crate's original GTK4 window still builds but nothing opens it. Shares a saved network to a phone as a QR code, without the passphrase ever leaving the daemon. No VPN, by design |
| **Liseuse** (`config/liseuse/`) | Centralised reading on SUPER+F — a fuzzel picker over `~/Livres` plus the folders in `sources.conf` (course PDFs stay in `~/courses`), ranked so the book you're mid-way through comes first. Every "is a book already open?" question is scoped to the current workspace, so the key never yanks you to another desk and a book left elsewhere keeps its own fullscreen. Renders through zathura + `zathura-pdf-mupdf`, one engine for PDF/EPUB/MOBI/CBZ, recolored dark — ink from `colors.lua`, paper pure black rather than the palette's near-black, since a document is the whole screen for an hour and nothing should sit between the letters and the dark. Markdown too, rendered to a GFM page first (`md2pdf.py`: python-markdown + Pygments + WeasyPrint), cached, with links between documents still working. Light and dark both follow the desktop's `color-scheme`, and page geometry is derived from the focused monitor rather than hardcoded, so Space turns a page and re-fits it on any screen. Rows carry a format icon and are searched by full path, so `md` or `dotfiles keybind` narrows a 51-document library. `liseuse convert` turns Word/PowerPoint documents into a PDF beside the original, where the scan picks it up. A book opens as an ordinary window — fullscreen is `Super + Shift + F` when you want it, not a default that has to be undone — and a session quiets notifications and takes a D-Bus idle inhibitor, then restores exactly what it found |
...
| **Boot** (`config/boot/`) | The two things that own the screen before the desktop exists. **Plymouth**: a theme with a pulsing word mark, one thin macOS-style progress bar and the boot log a line at a time, on black, all in JetBrains Mono — written because the machine drew *nothing* between GRUB and the greeter, the renderers package being missing so no theme could have. **Login**: greetd + tuigreet on VT1, with the console itself rasterised into JetBrains Mono (a Linux VT draws a PSF bitmap font, not a fontconfig family) — and a repair unit that re-applies the keymap after the splash quits, because `systemd-vconsole-setup` drops the keymap whenever `setfont` fails. `preview.sh` shows the splash on a spare VT without rebooting; `simulate.py` renders its motion to a video, reading every constant out of the theme so it cannot drift |
| **Rofi / SwayNC / dunst / systemd services** | Launcher, notifications, and background daemons (OLED-protection wallpaper slideshow) |
