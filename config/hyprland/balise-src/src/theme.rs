...
/* Real compositor blur behind Balise again (third pass -- see
   hypr/windowrules.lua's header comment), paired THIS time with a
   lighter, more translucent fill instead of Orbit's near-opaque 0.96/
   0.97: rgb(40,40,45) is a genuine mid-gray rather than near-black, at
   0.72 opacity so the blur actually shows through the glass instead of
   being masked by an almost-solid layer sitting on top of it -- that
   was the point of the previous two attempts reading as "dull/dark"
   rather than "glass".
...
.balise-panel-inner {
    background-color: rgba(40, 40, 45, 0.72);
