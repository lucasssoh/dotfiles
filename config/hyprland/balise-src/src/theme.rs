...
/* Border: the SAME "curved glass" treatment as Hyprland's active-window
   ...
   Fill opacity 0.88: Balise DOES get real compositor blur behind it (see
   the "balise" layer_rule in hypr/windowrules.lua -- a fresh, explicit
   request for a "real glass" look, unlike Orbit which had blur removed
   earlier this session). 0.88 is deliberately more transparent than
   Orbit's old 0.97 (which was specifically compensating for *lacking*
   blur) so the blur is actually visible through the fill -- close to
   swaync's .control-center (0.90), a touch lower since Balise was asked
   to read as glass first. */
.balise-panel {
    border: 1.5px solid transparent;
    border-radius: 16px;
    background-image:
        linear-gradient(rgba(12, 12, 14, 0.88), rgba(12, 12, 14, 0.88)),
        linear-gradient(
            135deg,
