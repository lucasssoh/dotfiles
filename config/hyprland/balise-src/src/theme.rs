...
/* No compositor blur behind Balise (tried it, removed again -- see
   ...
   margin: 3px is the border's thickness (the gradient showing through
   from .balise-panel behind it) -- bumped from an invisible 1.5-2.5px
   attempt to something that actually reads at this panel's scale, per
   direct feedback. border-radius 13px (16 - 3) keeps the inner corner
   concentric with the outer one instead of looking squarer. */
.balise-panel-inner {
