...
/* Border: the SAME "curved glass" treatment as Hyprland's active-window
   ...
   No compositor blur behind Balise (tried it, removed again -- see
   hypr/windowrules.lua's header comment: a flat blurred backdrop read as
   dull/dark, not "glass"). Fill opacity 0.96, back up near Orbit's own
   0.97 for the same reason Orbit is that high: without blur behind it,
   the glass material has to be carried entirely by the fill + border,
   and a lower opacity here just shows an unblurred, distracting hint of
   whatever's behind instead of reading as glass.

   Border width bumped 1.5px -> 2.5px and every gradient stop raised
   (most visibly the tail end, 0.06 -> 0.18) so the ring reads clearly
   all the way around instead of fading to near-invisible past the
   top-left corner -- the previous values were "correct" in principle
   (same ramp as Hyprland's own window border) but too subtle to actually
   see at this panel's scale, per direct feedback. */
.balise-panel {
    border: 2.5px solid transparent;
    border-radius: 16px;
    background-image:
        linear-gradient(rgba(12, 12, 14, 0.96), rgba(12, 12, 14, 0.96)),
        linear-gradient(
            135deg,
