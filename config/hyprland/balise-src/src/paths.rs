...
/// orbit/prisme/roue) -- overridable via `BALISE_CONFIG_DIR` so the crate
...
/// The daemon's Unix domain socket. Deliberately a DISTINCT name from
/// Orbit's `orbit.sock` -- both daemons run side by side during Balise's
/// build-out (Orbit stays the active, bar-wired tool until Balise is
/// validated), so they must never contend for the same socket path.
