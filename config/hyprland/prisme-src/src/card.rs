        ...
        fn snapshot(&self, snapshot: &gtk4::Snapshot) {
            ...
            if focus > 0.15 {
                ...
                if let Some(wallpaper) = self.wallpaper.get() {
                    let name_layout = widget.create_pango_layout(Some(&wallpaper.name));
                    name_layout.set_ellipsize(pango::EllipsizeMode::Middle);
                    name_layout.set_width(((w - 32.0).max(0.0) * pango::SCALE as f64) as i32);
                    let text_color =
                        gdk::RGBA::new(0.949, 0.949, 0.969, alpha);

                    snapshot.save();
                    snapshot.translate(&graphene::Point::new(16.0, (h - 46.0) as f32));
                    ...
                    if dw > 0 && dh > 0 {
                        let dims_layout =
                            widget.create_pango_layout(Some(&format!("{dw}\u{00d7}{dh}")));
                        let dims_color = gdk::RGBA::new(0.557, 0.557, 0.576, alpha);
                        snapshot.save();
                        snapshot.translate(&graphene::Point::new(16.0, (h - 26.0) as f32));
...
fn parallelogram_path(w: f64, h: f64, skew: f64) -> gsk::Path {
    ...
    builder.move_to(skew as f32, 0.0);
    builder.line_to(w as f32, 0.0);
    builder.line_to((w - skew) as f32, h as f32);
    builder.line_to(0.0, h as f32);
    builder.close();
    builder.to_path()
}
