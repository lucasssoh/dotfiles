...
local function emit(types)
    ...
    local L = {
        "@startuml",
        "' Generated by :PumlFromJava from code : to review, DO NOT BELIEVE",
        "set separator none",
...
function M.generate(dir)
    ...
    if #files == 0 then
        vim.notify("PumlFromJava : no .java under " .. dir, vim.log.levels.WARN)
        return
    end
    table.sort(files)
    ...
    vim.notify(string.format("PumlFromJava : %d types, %d files (buffer not saved : <A-p> to view, :w to save)",
        #types, #files))
end
